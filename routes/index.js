...
router.post("/checkupdatecurso/:id",async function (req, res, next) {
  const course = await professorDb.updateCourse(req.body.nombre,req.body.seccion,req.body.tipo,'I',req.params.id);
  console.log(course);
...
router.get("/vercursos", async function (req, res, next) {
  ...
  if (req.cookies.tokenUser) {
    ...
    const professor = await professorDb.getProfessorToCourse(token);
